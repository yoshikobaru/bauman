...
$APPLICATION->SetTitle('Попечительский совет МТ4');

$cmsImage = '';
$projectRow = function_exists('po_get_iblock_project_by_detail_url')
    ? po_get_iblock_project_by_detail_url('/projects/trustees/')
    : null;
if (!$projectRow) {
    $projectRow = [
        'detail_url' => '/projects/trustees/',
        'donation_name' => 'Попечительский совет МТ4',
    ];
} else {
    $projectRow['donation_name'] = $projectRow['donation_name'] ?? 'Попечительский совет МТ4';
}
if ($projectRow) {
...
$tplPath = $_SERVER['DOCUMENT_ROOT'] . '/local/templates/my_template/project_trustees.html';
